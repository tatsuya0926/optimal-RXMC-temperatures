...
function metropolis_sampler(config, β, N)
    ...
    for i in range_i
        for j in range_j
            ...
            cost = 2s * nb
            flip_prob = min(1.0, exp(-cost * β))
            coin = rand(Bernoulli(flip_prob))
            config[i,j] = coin * (-s) + (1 - coin) * s
        end
    end
...
function calc_gradient_no_constraints(
    N,
    β_replicas; 
    mcSteps=10^4, 
    eqSteps=10^3,
...
function calc_gradient(
    ...
    for i in (M-2):-1:1
        grad_tmp += grads[i+1]
        estimate_grads[i] = grad_tmp * (β_replicas[i+1] - β_replicas[i])
    end
    ...
    return estimate_grads, exchange_probs_means
end

function calc_round_trip_times(
    N,
    config,
    β_replicas;
    mcSteps=10^4
...
export initial_state, metropolis_sampler, calc_energy, calc_magnetization, calc_gradient_no_constraints, calc_gradient, calc_round_trip_times
